...
// Mount user-facing ticket endpoints (guest view + updates)
try {
  const userTicketingRouter = require('./routes/user-ticketing');
  userTicketingRouter(app, { TicketModel, ticketsContainer: null, inMemoryTickets, USE_COSMOS_DB: false });
  console.log('✓ User ticketing endpoints mounted');
} catch (e) {
  console.warn('User ticketing routes not loaded:', e.message);
}
